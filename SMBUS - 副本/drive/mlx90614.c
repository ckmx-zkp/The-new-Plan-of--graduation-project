...
#define SMBUS_SDA_PIN()	    SMBUS_PORT->IDR & SMBUS_SDA //¶ÁÈ¡Òý½ÅµçÆ½


//extern void SMBus_Delay(__IO uint32_t nTime);
...
uint8_t SMBus_ReceiveBit(void)
{
    ...
    SMBUS_SDA_H();          //Òý½Å¿¿Íâ²¿µç×èÉÏÀ­£¬µ±×÷ÊäÈë
...
void SMBus_Init()
{
    ...
    /*ÅäÖÃSMBUS_SCK¡¢SMBUS_SDAÎª¼¯µç¼«¿ªÂ©Êä³ö*/
...
repeat:
        ...
        if(SMBus_SendByte(slaveAddress))//Send SlaveAddress ×îµÍÎ»Wr=0±íÊ¾½ÓÏÂÀ´Ð´ÃüÁî
        {
        ...
        if(SMBus_SendByte(slaveAddress+1))	//Send SlaveAddress ×îµÍÎ»Rd=1±íÊ¾½ÓÏÂÀ´¶ÁÊý¾Ý
        {
            goto	repeat;             	//Repeat comunication again
        }
